    ...
    function areActiveRoutes(array $routes, $output = "active")
    {
        foreach ($routes as $route) {
            ...
            if (Route::currentRouteName() == 'attributes.edit') {
                ...
                if ($route == 'product.attributes.edit' && $attribute->content_type == 'App\Product') return $output;
                if ($route == 'seller.attributes.edit' && $attribute->content_type == 'App\Seller') return $output;
            }
        }
    }
}
...
function translate($key, $lang = null)
{
    ...
    $translation_def = Cache::remember($key . $lang, 60, function () use ($key) {
        return Translation::where('lang', env('DEFAULT_LANGUAGE', 'en'))->where('lang_key', $key)->first();
    });
    ...
    $translation_locale = Cache::remember($key . $lang, 60, function () use ($key, $lang) {
        return Translation::where('lang_key', $key)->where('lang', $lang)->first();
    });
