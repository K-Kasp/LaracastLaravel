...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <a class="d-flex align-items-center mb-3" href="{{ route('attributes.slug_index', ['slug' => 'sellers']) }}">
              ...
              <div class="form-group mb-3">                
                @if ($attribute->type == 'dropdown' || $attribute->type == 'checkbox')
                  <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a class="btn btn-soft-primary" href="{{route('attribute_value.create', ['attribute_id'=>$attribute->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}">
                    ...
                    <tbody>
                      @foreach($attribute->attribute_values as $key => $value)
                        <tr>
                          ...
                          <td>{{$value->values}}</td>
                          <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('attribute_value.edit', ['id'=>$value->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                              <i class="las la-edit"></i>
                            </a>
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('attribute_value.destroy', $value->id)}}" title="{{ translate('Delete') }}">
