...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <a class="d-flex align-items-center mb-3" href="{{ route('attributes.edit', ['id'=>$attribute_value->attribute->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}">
        ...
        <div class="card-body p-0">
          <ul class="nav nav-tabs nav-fill border-light">
            @foreach (\App\Language::all() as $key => $language)
              <li class="nav-item">
                <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3" href="{{ route('attribute_value.edit', ['id'=>$attribute_value->id, 'lang'=> $language->code] ) }}">
          ...
          <form class="p-4" action="{{ route('attribute_value.update', $attribute_value->id) }}" method="POST">
