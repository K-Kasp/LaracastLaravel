        ...
        <table class="table aiz-table mb-0">
            ...
            @if(!is_null($shop_audits))
                @foreach($shop_audits as $audit)
                            ...
                            @switch($key)
                                ...
                                <td>
                                    @foreach($arr_value as $status=>$value)
                                        ...
                                        @if($status == 'new')
                                            <strong>New Value = {{\App\Attribute::find($value)->name}}</strong> <br>
                                        @elseif($status == 'old')
                                            <strong>Old Value = {{\App\Attribute::find($value)->name}}</strong>
                                        @endif
                                    @endforeach
                                </td>
                    ...
                    <td>{{\App\Attribute::find(\App\Models\AttributeValue::find($arr_value['new'])->attribute_id)->name. translate(' Value Changed')}}</td>
                    <td>
                        @foreach($arr_value as $status=>$value)
