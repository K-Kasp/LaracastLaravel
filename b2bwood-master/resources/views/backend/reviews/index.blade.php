    ...
    <div class="card-header">
        <div class="row flex-grow-1">
            ...
            <div class="col-md-6 col-xl-4 ml-auto mr-0">
                ...
                <form id="sort_by_content_type" action="{{ route('reviews.index') }}" method="GET">
                    <div class="" style="min-width: 200px;">
                        <select class="form-control aiz-selectpicker" name="content_type" id="content_type" onchange="filter_by_content_type()">
                            ...
                            <option value="App\Shop" {{$content_type == 'App\Shop' ? 'selected' : ''}}>{{translate('Company Reviews')}}</option>
                            <option value="App\Product" {{$content_type == 'App\Product' ? 'selected' : ''}}>{{translate('Product Reviews')}}</option>
                            <option value="App\Seller" {{$content_type == 'App\Seller' ? 'selected' : ''}}>{{translate('Seller Reviews')}}</option>
                            <option value="App\Events" {{$content_type == 'App\Events' ? 'selected' : ''}}>{{translate('Event Reviews')}}</option>
                        </select>
                    </div>
                </form>
            </div>
